...
class PromptBindingTests(unittest.TestCase):
    def test_runtime_matches_declared_assessment_revision(self):
        specification = Path(__file__).resolve().parents[1] / 'astra-preparation/astra-assessment-instructions-v2.json'
        proposed = json.loads(specification.read_text(encoding='utf-8'))['roles']
        self.assertEqual(ASSESSMENT_PROMPT_VERSION, 'astra-assessment-instructions-v2')
