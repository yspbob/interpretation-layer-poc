...
The first pilot uses evidence-backed historical tasks and reports possible prior model familiarity. It compares methods on these public projects; it does not demonstrate inference solely from previously unseen code. Private rule-changing variants are optional later work and need independently justified validation before admission. Their absence does not relax the isolation or sealed-assessment requirements: even a public-source historical task has role-specific inputs and protected evaluation records.
