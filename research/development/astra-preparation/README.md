...
Astra remains the model for the controlled workflow. On 14 September, the user added [Fable 5.1 through Claude Desktop](FABLE-DESKTOP.md) as a supplementary guidance assessor using the included Max allowance. Two public H06 development reviews and two public access diagnostics have completed at High effort. The desktop procedure has a bounded rehearsal, and its [48 response qualification package is now frozen](FABLE-QUALIFICATION-READINESS.md). All scheduled inputs preserve the original permitted content and fit the rehearsed byte envelope. No model has qualified and no experimental guide has been assessed. The Astra schedule is unchanged; no API spending has occurred.
