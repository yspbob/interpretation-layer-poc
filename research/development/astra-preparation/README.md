...
Astra remains the model for the controlled workflow. On 14 September, the user added [Fable 5.1 through Claude Desktop](FABLE-DESKTOP.md) as a supplementary guidance assessor using the included Max allowance. Two public H06 development reviews and two public access diagnostics have completed at High effort. The desktop procedure is specified and rehearsed on the diagnostic inputs; the actual qualification packet format and 48 response schedule still need to be frozen. No model has qualified and no experimental guide has been assessed. The Astra schedule is unchanged; no API spending has occurred.
