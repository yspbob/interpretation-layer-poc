...
- The first trial has three functional phases under plan section 1A: reconstruct guidance, test its use, then test interaction. The immediate research step is selecting guidance cases and building the model-free restricted drafting, verification and independent guidance assessment path recorded in PROJECT_STATE.md. Coding execution and the interactive checker are later phase gates, unless an exposed Phase 1 tool actually requires execution containment. Component contracts and the H04 scripted case are development material, not model qualification. Zero blinded runs have been completed. Neither judge qualification nor experimental benefit has been demonstrated.
