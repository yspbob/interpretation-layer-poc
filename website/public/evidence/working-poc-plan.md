...
Plan ID: **pilot-draft-2026-09-09.4**
...
- **Mediate communication externally.** Permit only declared, schema-validated exchanges through a controller outside the agent environments. Bind each exchange to its run, role and checkpoint. Deny other network access by default, including local networks, host services, metadata endpoints and indirect access through supporting services. Keep model credentials in the external logged gateway. A compromised workload or supporting service must not by itself grant wider network access or access to another role.
- **Prepare dependencies before execution.** Pin and provision dependencies in advance and make them read-only during runs. Do not provide a live package service, arbitrary URL fetcher or shared writable registry to experimental agents. A required dependency change stops the attempt for a recorded environment revision; it does not grant ad hoc internet access.
