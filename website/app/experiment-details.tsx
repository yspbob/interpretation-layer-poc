...
export function ExperimentDetails(){return <>
  <section className="section-block" id="method">
    <div className="section-heading"><span className="section-no">04</span><div><span className="eyebrow">THE EXPERIMENT, STEP BY STEP</span><h2>How I intend to run the experiment</h2></div></div>
    ...
    <ol className="method-steps" aria-label="The four steps in one experiment">
      <li className="method-step">
        ...
        <div className="method-step-content">
          <p>Case preparation starts with a project rule supported by inspectable evidence and a code-change task where that rule matters. Before an agent attempts it, the reference assessment must distinguish a correct change, a real mistake and a valid exception. That assessment stays outside the agent’s workspace.</p>
          <DetailGroup><Disclosure id="choose-case" title="How are tasks selected and reference answers established?">
            <p>Each case starts from a published requirement or decision, with its scope checked against the implementation. If the guide and code disagree, the assessment must explain the disagreement. Copying the guide is not enough.</p>
      ...
      <li className="method-step">
        ...
        <div className="method-step-content">
          <p>A separate judge assesses the final code against the assessment prepared in step 1. The comparison covers correct changes, missed requirements, unnecessary objections and unfinished attempts. The cost assessment also counts the effort spent preparing guidance, reviewing work and answering questions.</p>
          ...
          <DetailGroup>
            <Disclosure id="reliable-judge" title="How will the judge’s reliability be established?">
              ...
              <p>The verifier, runtime checker and final judge each need to pass tests for their own role. Two AI models agreeing does not establish a reference answer. Without independent expertise to settle an ambiguous case, the judgement must rest on reproducible behaviour or clear published evidence, or remain unresolved.</p>
