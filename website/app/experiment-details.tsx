  ...
  <section className="section-block" id="method">
    ...
    <ol className="method-steps" aria-label="The four steps in one experiment">
      <li className="method-step">
        ...
        <div className="method-step-content">
          ...
          <DetailGroup><Disclosure id="choose-case" title="How are tasks selected and assessment criteria established?">
            <p>Each historical case starts from the code before the original change. The task and applicable rules must be tied to that time; later documentation cannot silently introduce a new requirement. We inspected one version of each repository to find potential test cases. For each task selected, the experiment will use the code as it stood before the original change. That may be a different version from the one initially inspected.</p>
      ...
      <li className="method-step">
        ...
        <div className="method-step-content">
          <p>The interpretation layer’s drafter examines the permitted project files and proposes guidance. A separate verifier checks whether the evidence supports each claim. Neither receives the later coding task. The accepted guidance is saved before those tasks are revealed and reused for their comparison. It is not prepared again for every agent attempt.</p>
          <p>The guide belongs to a specific code version and set of permitted sources. Different repositories, historical versions or source-access conditions may need different guides. The files the drafter may read are selected without using the hidden fix to guide that choice; otherwise, the selection itself could give away the answer.</p>
          ...
          <DetailGroup>
            <Disclosure id="allowed-evidence" title="Does the layer read the documentation, or work the rule out from code?">
              ...
              <p>Only claims admitted by the verifier enter the guide. Preparation stops when its budget or allowed revisions are exhausted. Both groups using guidance then receive the same claims accepted by the verifier. If no claims were accepted, both receive an empty guide. Missing or incorrect guidance remains part of the result. After the guide is saved, a separate assessment checks whether its claims are supported, where they came from and whether important requirements are missing. The verifier’s approval is not itself a score of guidance quality. The website’s examples are illustrations, not the guidance supplied to an experimental agent.</p>
            </Disclosure>
        ...
        <div className="method-step-content">
          ...
          <DetailGroup>
            <Disclosure id="reliable-judge" title="How will the assessments be shown to be reliable?">
              <p>The verifier and runtime reviewers need reliability tests for the decisions they make. Separate assessors judge the frozen guidance, final code and review interactions. Each reviewer or judge must be tested on the kind of decision it will make, using only the information it will receive in the experiment. The verifier cannot also provide the independent verdict on how well its own verification worked.</p>
              ...
              <p>Scores are audited during the pilot. If an assessor or its criteria change, it must pass validation again, and all affected implementations are rescored consistently. If a main-trial result is used to change how the work is judged, that result becomes part of developing the assessment. Fresh cases are then needed to test the revised method independently.</p>
            ...
            <Disclosure id="interpret-results" title="What conclusions would the comparison support?">
              <p>Useful guidance must be supported by evidence, improve later work and justify its cost. The guidance itself also needs assessment, so a correct code change does not conceal unsupported rules. A small, separate check gives the agent a known correct rule directly. This helps establish whether that task could benefit from good guidance at all.</p>
    ...
    <div className="method-preparation">
      ...
      <p>Before any model is called, tests must show that agents cannot use the prohibited access routes. Each reviewer and judge must then pass the tests for its role before the pilot relies on its decisions or scores. A small pilot then tests the full procedure before a main trial supports conclusions. Findings from the pilot inform the method and size of the main trial.</p>
