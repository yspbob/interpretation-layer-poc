...
export function ExperimentDetails(){return <>
  <section className="section-block" id="method">
    ...
    <p className="section-intro">This is the detailed procedure for the later coding phases. Phase 2 uses the first two groups; Phase 3 runs all three together. The sequence starts before guidance preparation because we must choose its intended tasks before seeing what the guide contains.</p>
    ...
    <ol className="method-steps" aria-label="The four steps in one experiment">
      ...
      <li className="method-step">
        ...
        <div className="method-step-content">
          <p>The interpretation layer’s drafter examines the project files it is allowed to read and proposes guidance. A separate verifier checks whether the evidence supports each claim.</p><p>Neither receives the later coding tasks. The investigator chooses the intended tasks first. We save the verifier’s guide before showing those tasks to the coding agents, then reuse it for the recorded comparisons. The drafter does not start again for every agent attempt.</p>
          ...
          <DetailGroup>
            <Disclosure id="allowed-evidence" title="Does the layer read the documentation, or work the rule out from code?">
              ...
              <p>Only claims accepted by the verifier enter the guide. Preparation stops when its budget or allowed revisions are exhausted. The guidance group in Phase 2 receives that result. In Phase 3, both guidance groups receive the same result. If no claims were accepted, the guide is empty.</p><p>We retain that outcome and its cost. We do not select only guides that scored well in the independent assessment. Missing or incorrect guidance is part of what the experiment needs to measure.</p><p>We may stop or improve the method between phases. If we change it, we record the new version and prepare fresh guidance where needed. We cannot present selected successes from the old version as evidence of the whole method’s reliability.</p><p>After the guide is saved, a separate assessor checks its quality. That assessment asks whether its claims are supported, where they came from and whether important requirements are missing. The verifier cannot provide the independent verdict on its own work.</p><p>The website’s examples are illustrations. The actual guidance supplied to an experimental agent will be prepared under this procedure.</p>
            </Disclosure>
            ...
            <Disclosure id="isolation" title="How is withheld assessment material kept out of the inputs?">
              <p>Each agent receives only its assigned files and history. A new independent attempt starts separately, with no access to previous attempts, shared memory or our research notes. Historical solutions and assessment answers stay out of the coding and review environments. The <a className="text-link" href="#technical-setup">technical setup</a> explains the execution boundaries and permitted communication routes.</p>
