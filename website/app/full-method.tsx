...
export function FullMethod(){return <>
  <section id="example" className="section-block">
    ...
    <p className="section-intro">For a proposed change, the layer should explain which existing code can be reused and which project rules apply. Its answer should show the evidence and any valid exceptions. It must distinguish an approved decision from a pattern observed in code or a question still awaiting review.</p>
    <p className="section-intro">In the interactive phase, an agent can ask about its approach before proceeding. If the evidence cannot settle a decision needed for the task, the attempt stops. Production would involve an owner; this POC has no owner to supply that answer.</p>
    <DetailGroup><Disclosure id="guidance-examples" title="See examples of the guidance an agent could receive" summary="How this applies to change history, conflicting documentation and questions the code cannot answer.">
      <p>NetBox is an application with public source code used to document network infrastructure, including devices and their connections. It is one of the four candidate projects we screened. The trial repository has not been chosen.</p>
  ...
  <section className="section-block">
    <div className="section-heading"><span className="section-no">02</span><div><span className="eyebrow">THE IDEA BEHIND THE LAYER</span><h2>How the production proposal differs from this trial</h2></div></div>
    ...
    <DetailGroup><Disclosure id="production-scope" title="Where does this POC differ from production?" summary="Owner approval, repeated use, maintenance and the way success is measured.">
      ...
      <p>Production would apply the layer to current engineering work. This POC repeats tasks under two conditions in Phase 2 and three in Phase 3. Each comparison controls the inputs and review rules, then assesses the results independently. These controls help us compare the methods fairly. A production team would choose a workflow suited to its own work.</p>
    ...
    <p className="caption">In both coding phases, every group receives review. The <a className="text-link" href="#during-work">detailed method</a> explains the checks before editing, during implementation and at final submission.</p>
  </section>
  ...
  <section className="section-block" id="comparison">
    ...
    <p className="section-intro">The table below explains what help each group receives. Every preparation result remains in its declared comparison, including an empty guide and the cost of producing it.</p>
  ...
  <ExperimentDetails/>
  <TechnicalSetup/>
<details className="after-trial" id="what-could-follow"><summary>What could follow the first trial?</summary><p>If the first trial justifies it, a separate confirmation study would freeze the method and use fresh cases that did not shape it. Broader repositories, models, production approval and maintenance remain possible later studies.</p></details></>}
