...
export default function Experiment(){return <Shell active="experiment">
  <div className="page-heading"><span className="eyebrow">AI ENGINEERING PLAYBOOK / CHAPTER 4</span><span className="reading-time">Why we are running this experiment</span></div>
  <section className="intro">
    ...
    <aside className="hypothesis">
      ...
      <div className="hypothesis-explanation">A capable agent may already work out the right approach from the sources. Preparing guidance and consulting it takes time and money. We need to find out whether these steps prevent enough mistakes to justify the effort.</div>
  ...
  <section id="example" className="section-block">
    ...
    <DetailGroup><Disclosure id="guidance-examples" title="See examples of the guidance an agent could receive" summary="How this applies to change history, conflicting documentation and questions the code cannot answer.">
      <p>NetBox is an open-source application used to document network infrastructure, including devices and their connections. It is one of the four projects we are examining in this study.</p>
  ...
  <section className="section-block">
    ...
    <div className="working-loop">
      ...
      <p>Giving every group a review makes the comparison more useful. If the interactive group does better, we can examine whether the layer’s specific questions helped, with ordinary review already available to the other groups. All reviewing and questioning would count towards the agreed resource budget.</p>
  ...
  <section className="section-block">
    ...
    <p className="section-intro">We should give agents the same change to make under three conditions. This would tell us whether preparing guidance helps by itself, and whether consulting the layer and checking the plan adds a further benefit. The pilot procedure above is our proposed starting point; the exact prompts, budgets and checks still need validation before the trial.</p>
    <div className="comparison comparison-three">
      ...
      <div><span className="eyebrow">WITH GUIDANCE AND INTERACTION</span><h3>The agent checks its proposed actions</h3><p>The agent receives the same guidance, consults the layer about its plan and responds to checks at agreed points. We record whether those interactions prevent mistakes or add unnecessary work.</p></div>
    </div>
    <p className="caption">All three conditions need the same permitted evidence and comparable resources, including opportunities for review. We will count the cost of preparing guidance and any additional checking. The two guidance conditions must use the same frozen rules, so a difference between them is not simply caused by one receiving better guidance.</p>
  </section>
