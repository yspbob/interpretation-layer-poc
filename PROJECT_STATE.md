...
For website discussion, bank comments and proposed changes in the conversation until the user explicitly requests application or release. Do not edit, build or publish after each comment. The 9 September instruction to review both pages, adjust and release authorises the current batch. Match the playbook's authorial voice: use I/my for the author's recommendations and judgement, name the responsible component for procedures, and use we only for a real group. Do not imply that automated checks were independent human review.
